...
export function normalizeConfig(raw = {}) {
  ...
  const missionCards = Array.isArray(raw.missionCards)
    ? raw.missionCards.map((c) => ({
        ...
        // Whether the ayah block shows on the mission card. Default true so
        // events created before this flag keep showing their ayah.
        show_ayat: c.show_ayat !== false,
        ayat: {
...
export async function saveReport({ slug, date, groupKey: gk, group, fields, mission, ref, room = null }) {
  const submittedAt = new Date().toISOString();
  const record = { group, room, fields, mission, ref, submittedAt };
...
// Delete every key matching a prefix (SCAN-based, cluster-safe batches).
...
// Admin: reset an event's draw state to a clean slate.
//   offline: every mission count back to cap (the bag) + clear name locks.
//   online:  clear the opener counter + per-group card locks so room numbering
//            restarts from 1.
// Room/lock keys are cleared across ALL dates (not just today's) so a reset works
// regardless of the UTC day boundary between when players joined and when the
// admin resets. Client-side claims (himmah_claim_<slug>) are intentionally left,
// so already-joined players keep their room; only new joiners restart at Room 1.
export async function resetPool({ slug, cfg }) {
  ...
  await delByPrefix(`event:${slug}:group:`); // offline name locks (all dates)
  if (cfg.player_per_group > 1) {
    await delByPrefix(`event:${slug}:openers:`); // room counter(s), all dates
    await delByPrefix(`event:${slug}:grp:`);      // group -> card locks
    await delByPrefix(`event:${slug}:opener:`);   // per-player slot keys
  }
  return { ok: true };
}
