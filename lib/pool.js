...
// Admin: reset an event's draw state for the current day.
//   offline: every mission count back to cap (the bag).
//   online:  clear the opener counter + per-group card locks so group numbering
//            restarts from 1 (offline-only reset would be a no-op online).
export async function resetPool({ slug, cfg, date = today() }) {
  const fields = {};
  ...
  if (cfg.player_per_group > 1) {
    await redis.del(openersKey(slug, date));
    await delByPrefix(grpPrefix(slug, date));
    await delByPrefix(`event:${slug}:opener:${date}:`);
  }
  return { ok: true };
}
