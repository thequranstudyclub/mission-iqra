...
// calls within a warm serverless instance. The SDK is imported dynamically
// (server-only) so this module stays browser-safe: events.html imports the slug
// helpers above from here, and a bare `@quranjs/api/server` specifier would fail
// to resolve in the browser and break the whole page.
let _client = null;
async function qfClient() {
  if (!_client) {
    const clientId = process.env.QF_CLIENT_ID;
    const clientSecret = process.env.QF_CLIENT_SECRET;
    if (!clientId || !clientSecret) throw new Error("qf_credentials_missing");
    const { createServerClient } = await import("@quranjs/api/server");
